...
topk_predictive_result_path = "data/output/domain_model_out/candidate_label/cjo22/BERTC/article_topk_6w.json"
topk = 3
y_pred_topk = []
y_pred = []
with codecs.open(topk_predictive_result_path, "r", "utf-8") as f:
    data = json.load(f)
    for d_i in data:
        y_pred_topk.append([int(x) for x in d_i[:k]])
        y_pred.append(int(d_i[0]))
