...
article_prompt = """
你是一个中国法律专家，擅长进行法律法条预测。
案件事实描述包括三个主要描述：主观动机、客观行为和事外情节。
用<>括起来的是本案事实，用---括起来的是与本案内容类似的3个案件。请通过主观动机，客观行为与事外情节三个方面,理解与比较用---括起来的3个案件与本案事实的异同，选择本案的相关法条。注意：请输出本案的相关法条，并结合类案给出选择本案相关法条的理由。
...
# 判断逻辑
1. 理解本案事实和3个与本案内容类似的案例；
2. 根据事实描述，通过主观动机，客观行为与事外情节三个方面,理解与比较3个案例与本案事实的异同；
3. 根据比较结果，选择本案相关的法条；

# 输出格式
1. 先根据判断逻辑一步一步地分析，陈述你的理由；
2. 最后输出你的答案，答案使用“$$”符号修饰，比如: $$第1条$$, $$第2条$$。

本案的相关法条是以下几个选项的其中之一：{options}
现在，请开始你的分析：
"""
